...
pub fn analyze(
    value: f64,
    range: (f64, f64),
    critical: Option<CriticalThresholds>,
) -> AnalysisResult {
    let (low, high) = range;
...
mod tests {
    ...
    fn critical_thresholds_take_precedence() {
        let critical = Some(CriticalThresholds {
            low: Some(2.5),
            high: Some(6.5),
        });
        assert_eq!(
            analyze(2.5, (3.5, 5.0), critical).status,
            Status::CriticalLow
        );
        assert_eq!(
            analyze(2.4, (3.5, 5.0), critical).status,
            Status::CriticalLow
        );
        assert_eq!(analyze(3.0, (3.5, 5.0), critical).status, Status::Low);
        assert_eq!(
            analyze(6.5, (3.5, 5.0), critical).status,
            Status::CriticalHigh
        );
        assert_eq!(
            analyze(6.6, (3.5, 5.0), critical).status,
            Status::CriticalHigh
        );
